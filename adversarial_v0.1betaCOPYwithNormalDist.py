...
for i in range(1):
    ...
    for k in range(1000):
        ...
        X = stats.truncnorm((1-w_s_mu) / 1.33, (20-w_s_mu) / 1.33, loc=w_s_mu, scale = 0.5)
        ...
        #plt.clf()
    utopia = k + 1 - dystopia
    n , bins, patches = plt.hist(q,25,normed=1, facecolor = 'green', alpha=0.75)
    plt.show()
    #np.savetxt((str(i+1) + "WITNESSES_PAPERPARAMS_100RUNS_outcomes.npy"), outcomes, fmt='%1.3f', delimiter=' ')
    np.savetxt((str(i+1) + "TRUNC_NORM_WITNESSES_results.npy"), np.array([utopia, dystopia]), fmt='%1.3f', delimiter=' ')
